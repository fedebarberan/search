// Version 2.2 - Fixed SHA-256 integrity, Gemini icon
document.addEventListener('DOMContentLoaded', () => {
    const faviconUrls = {
        ...
        'gemini': 'https://www.google.com/favicon.ico', // Changed from uxwing
    ...
    forms.forEach(form => {
        ...
        counter.textContent = `${form.max}/${form.max}`;
        input.addEventListener('input', () => {
            const remaining = form.max - input.value.length;
            counter.textContent = `${remaining}/${form.max}`;
        });
    });
    ...
    function logStats(type, value, query = '') {
    ...
    function handleSubmission(formId, buttonId, type) {
            ...
            if (input.value.trim()) {
                ...
                fetch(form.action, {
                    ...
                    if (response.ok) {
                        ...
                        logStats('submission', type);
                        setTimeout(() => {
                            form.parentElement.parentElement.style.display = 'none';
                            successMsg.parentNode.replaceChild(button.cloneNode(true), successMsg);
                            button.disabled = false;
